...
* Riportami dentro i tag <shell></shell> tutti i comandi a terminale (windows, integrato in VS Code) necessari per far funzionare le nuove modifiche. Quando vedi che l'app si interfaccia con dei db dammi gli appositi comandi di generate e migrazione se le modifiche lo rendono necessario. Di default dammi i comandi per aggiornare il sistema in locale, perché di base lavorarerò più che altro in locale. Ad esempio se sto usando drizzle su un D1 all'interno del tag shell aggiungerai:
