...
*   **Pagine legali da non indicizzare:** Pagine Terms, Privacy, Cookies DEVONO avere `robots: { index: false }` ed essere rimosse dalla sitemap (MAI bloccate con Disallow in `robots.ts`, o Google non leggerà il noindex).
...
*   **Regole su Cloudflare (anti-duplicate content):** Conviene sempre impostare su Cloudflare due Rules, che si trovano già pronte con i template: 1. Redirect da www a senza www, 2. Redirect 301 da http:// ad https:// (modifica anche questa impostazione: Cloudflare dashboard → SSL/TLS → Edge Certificates: verifica "Always Use HTTPS" ON). Se sospetti che questo non sia stato fatto, se ci sono segnali a riguardo o se l'utente sta facendo un check generale prima della pubblicazione, first deploy, di un sito/app, allora ricordargli queste due regole da applicare.
Non fidarti di Cloudflare da solo; aggiungi SEMPRE nel middleware un check su `x-forwarded-proto` con redirect 308 per garantire l'URL canonico. CRUCIALE: avvolgi questo check in `if (process.env.NODE_ENV === 'production')`, altrimenti causerai un redirect loop infinito lavorando in `localhost`.
*   Se emerge problema da Google search console **"Pagine rilevate ma non indicizzate":** controlla SEMPRE per primo il linking interno (Header/Footer/Home) verso quelle rotte; su siti/template cloni scalabili aggiungi anche Article+BreadcrumbList JSON-LD su ogni pillar page fin dal boilerplate.
*   **Linking interno tra pillar page** (problema: Content silo senza cross-link): se generi N pillar page da un array di post, verifica SEMPRE che ogni pagina linki ad altre correlate (related/next articles), non solo l'indice. Un template di pillar page dovrebbe includere questo blocco di default fin dalla creazione.
*   **Allineamento Trailing Slash:** L'URL canonico dichiarato nei Metadata (`page.tsx`) e quello generato in `sitemap.ts` DEVONO essere identici carattere per carattere. Se la root ha la barra finale (`https://dominio.it/`) nel canonical, deve averla anche nella sitemap. Il mismatch genera duplicati in SERP.
