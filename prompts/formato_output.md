...
* Ciò che è contenuto in <original> è assolutamente FEDELE al file allo **stato attuale**. 
* ⚠️ **REGOLA DELLE ITERAZIONI (CRITICA):** Io applico SEMPRE i tuoi output XML sul mio codice locale, istantaneamente. Anche se ti dico che una tua modifica "non va bene" o "ha un errore" e ti chiedo di correggerla, **il mio file locale contiene già la tua modifica errata appena applicata**. Pertanto, il tuo successivo blocco `<original>` dovrà cercare il codice *esattamente come lo hai scritto tu nel tuo ultimo XML*, NON come era nel file di partenza. Aggiorna sempre la tua mappa mentale dello stato del file dopo ogni tua singola patch. Unica eccezione: quando ti dico esplicitamente "non ho applicato la patch" o "ho fatto undo".
* ⚠️ VINCOLO DI FEDELTÀ ESTREMA: Quando compili il tag <original>, non ricostruire il codice a memoria, non riassumere, non presumere che sia in un certo modo. Devi eseguire una ricerca testuale nell'ultima versione nota (che include le tue eventuali ultime modifiche!), individuare il blocco esatto di righe e copiarlo carattere per carattere, **inclusi i commenti**. Se il contenuto di <original> non è identico al 100% al file sorgente attuale, l'intera operazione fallirà. Gli stessi principi valgono anche per quando con <file> decidi di fare un FULL REWRITE: se stai riscrivendo un file che già esisteva riporta come stanno tutte le parti non oggetto di modifica, **inclusi i commenti**.
