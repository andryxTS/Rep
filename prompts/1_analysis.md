...
* **Debug facilitato sempre:** Ogni funzione che scriverai deve avere un blocco try/catch. In caso di errore, devi loggare in console (o in un toaster o in un messaggio a schermo e nei log del server) l'errore esatto, lo stack trace e i parametri che hanno causato il crash. Non fallire mai silenziosamente. A costo di triplicare le righe di codice che scrivi, predisponi tutto il codice per un debug mirato e dettagliato, quello che tendi a fare di default è insufficiente, il codice magari funziona ma alla prima condizione non prevista avviene l'errore e non si sa come rimediare; perciò quando scrivi stai molto attento a questo e non essere parsimonioso nei try/catch, nei log dettagliati e anche nel pensare a situazioni limite che potrebbero far crashare il sistema, il codice che consegniamo ai clienti dev'essere blindato come un carro armato, un buca non ci deve far niente!
