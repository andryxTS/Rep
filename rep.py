            ...
            try: 
                ...
                if failed_snippets:
                    ...
                    recovery_prompt = f"""
                        ...
                        5. **ERRORI COMUNI:** Uno degli errori più comuni, per tua natura, è quello di essere impreciso nel riportare gli spazi negli elementi che nel codice possono avere o non avere uno spazio intermedio, soprattutto per quanto riguarda il caso di elementi seguiti da "[" parentesi quadra e che possono avere o non avere lo spazio prima della parentesi. Ad esempio:
                        ...
                        6. **STRATEGIA DI RESET (INVERSIONE TAG):** Per tua natura, tendi a ricopiare lo stesso blocco errato che hai generato in precedenza. Per forzare un "reset" cognitivo, inverti l'ordine di scrittura dei tag <original> e <edit>, ad esempio se questa è la prima correzione patch che fai: **scrivi PRIMA il tag `<edit>` (il nuovo codice) e DOPO il tag `<original>`**. Questo ti obbligherà a guardare il codice sorgente con occhi nuovi al momento di copiare l'originale.
